...
CREATE FUNCTION public.public_track_request(query_text text)
RETURNS TABLE(
  id uuid,
  request_number text,
  title text,
...
BEGIN
  ...
  IF cleaned ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN
    ...
    FROM maintenance_requests mr WHERE mr.id = cleaned::uuid LIMIT 1;
  ...
  SELECT mr.id, mr.request_number, mr.title, mr.description, mr.status::text, mr.workflow_stage,
         mr.created_at, mr.updated_at, mr.client_name, mr.location, mr.priority,
         mr.service_type, mr.sla_due_date, mr.rating, mr.channel
  FROM maintenance_requests mr WHERE upper(mr.request_number) = upper(cleaned) LIMIT 1;
  IF FOUND THEN RETURN; END IF;
